...
in Python, so one `cdk deploy` builds every AWS resource for you — no clicking around the console.
...
- The [**AWS CDK**](https://docs.aws.amazon.com/cdk/v2/guide/home.html) lets you write that
  infrastructure in a real programming language (here, Python) instead of hand-writing
  templates. `cdk synth` turns your code into a CloudFormation template; `cdk deploy` hands it
  to CloudFormation.
...
| `cdk synth`     | Turn the Python app into a CloudFormation template. Great for a dry run.                                         | [Apps & synth](https://docs.aws.amazon.com/cdk/v2/guide/apps.html)           |
...
1. **[Python 3.8+](https://www.python.org/)** — this project is an AWS CDK app written in Python.
   Install it directly or via a version manager like [mise](https://mise.jdx.dev/) or `pyenv`.
   Check with `python3 --version`.
2. **The AWS CDK Toolkit (CLI)** — the `cdk` command. Install it with `npm install -g aws-cdk`
   (it is a Node.js tool even for Python apps), and check with `cdk --version`.
   [CDK getting started](https://docs.aws.amazon.com/cdk/v2/guide/getting_started.html).
3. **The AWS CLI v2** — for credentials and for submitting jobs.
   [Install the AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html).
   Check with `aws --version`.
4. **AWS credentials** for your (sandbox) account, configured locally.
...
5. **Docker is *not* required.** The head-node image is built in the cloud by CodeBuild.

Project dependencies (`aws-cdk-lib`, `constructs`) are installed into a local Python virtual
environment from `requirements.txt` — see Quick start below.

---

## Quick start

From the `aws_batch_squared_py/` directory:
...
`CDK_DEFAULT_REGION` before deploying, e.g. `CDK_DEFAULT_REGION=us-east-1 cdk deploy`.
...
`aws_batch_squared_py/config.py`.
...
source .venv/bin/activate              # activate it (each shell)
pip install -r requirements.txt        # install dependencies
pip install -r requirements-dev.txt    # install test dependencies (pytest)

pytest                 # run the test suite

cdk bootstrap          # one-time per account/region
cdk synth              # synthesize CloudFormation (dry run)
cdk diff               # diff against the deployed stack
cdk deploy             # deploy to the default AWS account/region
cdk destroy            # tear down (the S3 bucket is retained by design)
```

The `app` command in `cdk.json` is `python3 app.py`, so the CDK CLI runs the Python app
directly — no build step is required before `synth`/`deploy` (make sure the virtualenv is
activated so `aws-cdk-lib` is importable).

> **Testing:** `pytest` runs the suite. `tests/unit/test_stacks.py` is currently a
...
```
app.py                                    # CDK app entrypoint: loads context, instantiates the stack
aws_batch_squared_py/
  config.py                               # context loading, defaults, and validation
  nextflow_batch_stack.py                 # orchestrator: wires nested stacks + SSM parameters
  vpc_stack.py                            # networking
  s3_stack.py                             # workflow bucket
  iam_stack.py                            # roles/policies
  launch_template_stack.py                # EC2 bootstrap (cloud-init user-data)
  batch_stack.py                          # compute environments + job queues
  nextflow_stack.py                       # Nextflow head-node job definition + role
  nextflow_ecr_stack.py                   # head-node image build (ECR + CodeBuild)
...
- **Right-size the head node.** The head node runs the Nextflow driver, not the heavy compute, yet
  `nextflow_stack.py` hardcodes `vcpus=4` / `memory=16384` / a `3600s` timeout. Surface these as
...
- **Add unit tests** with the CDK [assertions module](https://docs.aws.amazon.com/cdk/v2/guide/testing.html):
  `Template.from_stack(...)` checks per nested stack (queues, compute envs, IAM scoping, job-definition
  env vars) plus the validation branches in `aws_batch_squared_py/config.py`. These need no AWS account
  and run in milliseconds via `pytest`.
